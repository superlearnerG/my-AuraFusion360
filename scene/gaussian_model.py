...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.is_masked_activation = torch.sigmoid
    ...
    def get_is_masked(self):
        return self.is_masked_activation(self._is_masked)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self._is_masked = nn.Parameter(is_masked.requires_grad_(True))
    ...
    def removal_setup(self, training_args, mask3d):
        ...
        is_masked_sub = self._is_masked[mask3d].detach()
    ...
    def inpaint_setup(self, training_args, initial_gs_xyz=None, initial_gs_rgb=None):
        ...
        def initialize_new_features_with_initial_gs(original_features, initial_gs_xyz, initial_gs_rgb):
            ...
            opacities = self.inverse_opacity_activation(1.0 * torch.ones((initial_gs_xyz.shape[0], 1), dtype=torch.float, device="cuda"))
    ...
    def stop_grad(self, training_args, no_grad_part_length):
        ...
        self._is_masked = torch.nn.Parameter(torch.cat([set_requires_grad(self._is_masked[:no_grad_part_length], False), set_requires_grad(self._is_masked[no_grad_part_length:], True)]))
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._is_masked.shape[1]):
            l.append("is_masked_{}".format(i))
        return l
    ...
    def save_ply(self, path):
        ...
        is_masked = self._is_masked.detach().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, is_masked), axis=1)
    ...
    def load_ply(self, path):
        ...
        is_masked_names = [
            ...
            if p.name.startswith("is_masked")
        ]
        is_masked = np.zeros((xyz.shape[0], len(is_masked_names)))
        for idx, attr_name in enumerate(is_masked_names):
            is_masked[:, idx] = np.asarray(plydata.elements[0][attr_name])        
    

        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device="cuda").requires_grad_(True))
        self._is_masked = nn.Parameter(torch.tensor(is_masked, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._is_masked = optimizable_tensors["is_masked"]
    ...
    def densification_postfix(self, new_xyz, new_is_masked, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_is_masked = self._is_masked[selected_pts_mask].repeat(N, 1)
        ...
        self.densification_postfix(new_xyz, new_is_masked, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_is_masked = self._is_masked[selected_pts_mask]
        ...
        self.densification_postfix(new_xyz, new_is_masked, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation)
