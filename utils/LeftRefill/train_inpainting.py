...
from utils.pretrained_paths import require_stable_diffusion_inpaint_checkpoint, use_local_torch_home

use_local_torch_home()
STABLE_DIFFUSION_INPAINT_RELATIVES = {
    os.path.join("pretrained_models", "stable-diffusion-2-inpainting", "512-inpainting-ema.ckpt"),
    os.path.join("pretrained_models", "stable-diffusion-2-inpainting", "512-inpainting-ema.safetensors"),
    os.path.join("..", "pretrained_models", "stable-diffusion-2-inpainting", "512-inpainting-ema.ckpt"),
    os.path.join("..", "pretrained_models", "stable-diffusion-2-inpainting", "512-inpainting-ema.safetensors"),
}
...
def resolve_resume_path(path):
    if path in STABLE_DIFFUSION_INPAINT_RELATIVES or path.removeprefix("./") in STABLE_DIFFUSION_INPAINT_RELATIVES:
        return str(require_stable_diffusion_inpaint_checkpoint())
    return path
...
if __name__ == '__main__':
    ...
    if args.restore:
        ...
        if args.restore and getattr(model, 'save_prompt_only', False) and not os.path.exists(args.save_path + f'/{args.exp_name}/ckpts/last_resave.ckpt'):
            ...
            reload_weights = load_state_dict(str(sd_ckpt_path), location='cpu')
        ...
        if not args.no_restore:
            reload_weights = load_state_dict(resolve_resume_path(config['resume_path']), location='cpu')
            torch_init_model(model, reload_weights, key='none')
