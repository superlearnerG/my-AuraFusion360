...
def PILtoTorch_depth(pil_image, resolution):
    # print(pil_image.size)
    resized_image_PIL = pil_image.resize(resolution)
    resized_image_numpy = (np.array(resized_image_PIL)).astype(np.float32)

    resized_image = torch.from_numpy(resized_image_numpy) 
    # print(resized_image.shape, resized_image.dtype, resized_image.min(), resized_image.max())
    if len(resized_image.shape) == 3:
        return resized_image.permute(2, 0, 1)
    else:
        return resized_image.unsqueeze(dim=-1).permute(2, 0, 1) # (1 h w)