...
def masked_l1_loss(network_output, gt, mask):
    ...
    # mask is shape (h, w) ; turn to (3, h, w)
    mask = mask.bool().unsqueeze(0).repeat(gt.shape[0], 1, 1)
    loss = torch.abs((network_output[mask] - gt[mask]))
    loss = loss.sum() / loss.shape[0]
    return loss

def loss_is_masked_3d(features, predictions, k=5, lambda_val=2.0, max_points=200000, sample_size=800):
    """
