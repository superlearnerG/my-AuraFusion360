...
import time
from utils.marigold_process import MarigoldImageProcessor
...
from utils.utils.batchsize import find_batch_size
from utils.utils.ensemble import ensemble_depths
from utils.utils.image_util import (
    chw2hwc,
    colorize_depth_maps,
            ...
            if depth_conditioning and target_depth is not None and mask_align is not None and torch.any(mask_align):
                ...
                # if False:
                    with torch.set_grad_enabled(depth_conditioning):
                        ...
                        # Resize back to original resolution
                        depth_orig = resize(depth_orig, rgb_in.shape[2:], interpolation=InterpolationMode.BILINEAR, antialias=True).squeeze()
                        loss = l2_loss(target_depth.detach(), depth_orig)
                        mask_align = mask_align.detach().reshape_as(loss)
