...
from utils.graphics_utils import getProjectionMatrixForwardFacing
from utils.spiral_utils import get_spiral
...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
  ...
  for k in ['object_mask', 'depth', 'normal', 'color', 'depth_normal']:
    ...
    if k == 'color':
      ...
      file0 = os.path.join(input_dir, 'object_mask', f'{idx_to_str(0)}.{file_ext}')
    elif k == 'depth_normal':
    ...
    with media.VideoWriter(
      ...
      for idx in tqdm(range(num_frames)):
        ...
        if k == 'color':
          ...
          img_file = os.path.join(input_dir, 'object_mask', f'{idx_to_str(idx)}.{file_ext}')
        elif k == 'depth_normal':
...
def generate_path_spiral(viewpoint_cameras, n_frames=120, args=None):
  ...
  h, w = viewpoint_cameras[0].image_width, viewpoint_cameras[0].image_height
  FovX, FovY = viewpoint_cameras[0].FoVx, viewpoint_cameras[0].FoVy
